...
- [ ] Day 1 — Paul McWhorter Arduino Lessons: setup + first blink/digitalWrite examples
- [ ] Day 2 — digitalRead + a button; add debouncing
- [ ] Day 3 — analogRead + analogWrite: understand what PWM actually is
...
- [ ] Day 7 — Build the reaction-timer game (random delay → LED → button stops clock → prints ms to serial) — **Week 1 checkpoint, push to GitHub**

## Week 2 — ESP32

- [ ] Day 8 — Random Nerd Tutorials: ESP32 getting-started, flash first sketch
- [ ] Day 9 — Order ESP32-S3 + classic ESP32 if you haven't; meanwhile read ESP-IDF vs Arduino-core docs to understand the two layers
- [ ] Day 10 — WiFi basics: connect to your network, print IP to serial
...
- [ ] Day 15 — DroneBot Workshop: DC motor + L298N theory (learn it, but plan to not use it)
- [ ] Day 16 — SparkFun TB6612FNG hookup guide: wire the driver you'll actually use
- [ ] Day 17 — Drive one DC motor forward/backward at 5 different PWM speeds
- [ ] Day 18 — Add an encoder; write a function that turns the wheel exactly one revolution regardless of battery voltage (your first real closed loop)
- [ ] Day 19 — Stepper motors: wire a NEMA17 to an A4988 driver, run DroneBot's stepper demos
- [ ] Day 20 — Hobby servo vs smart bus servo vs stepper: write a one-page comparison in your own words — **Week 3 checkpoint**

## Week 4 — Sensors and the physical world

- [ ] Day 21 — Wire an IMU (MPU-6050 or BNO085); print raw pitch/roll to serial, watch it drift when still
- [ ] Day 22 — Implement a complementary filter (angle = a*(angle+gyro*dt) + (1-a)*accelAngle); watch the drift disappear
- [ ] Day 23 — Roger Labbe's Kalman filter notebooks: work through the g-h and discrete Bayes chapters
- [ ] Day 24 — HC-SR04 or VL53L0X: wire it, understand cone-angle limitations, log distance readings
- [ ] Day 25 — QTR reflectance array / TCRT5000 sensors: wire the line-sensor array for the line follower build

## Week 5 — Build the two robots

- [ ] Day 26 — Assemble line-follower chassis + wiring (ESP32/Arduino + driver + reflectance array)
...
- [ ] Day 29 — Assemble self-balancer chassis + wiring (motors w/ encoders + IMU)
